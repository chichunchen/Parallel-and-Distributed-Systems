	...
	for(rounds = 0; rounds < total_nodes - 1; rounds++) {
		for(unsigned int node = start; node < end; node++) {
			...
			for(unsigned int e = g.row_start[node]; e < g.row_start[node + 1]; e++) {
				...
				while (true) {
					int distance = g.node_wt[node].load(memory_order_relaxed) + g.edge_wt[e].load(memory_order_relaxed);
					int prev_distance = g.node_wt[dest].load(memory_order_relaxed);
	 
					bool pred_1 = distance == (g.node_wt[node].load(memory_order_relaxed) + g.edge_wt[e]);
					bool pred_2 = prev_distance == g.node_wt[dest].load(memory_order_relaxed);
	 
					if (pred_1 && pred_2) {
						if(prev_distance > distance) {
 							g.node_wt[dest].exchange(distance, memory_order_relaxed);
							// check if atomic swap success
							// if not success, then we continue to next loop
							if (g.node_wt[dest].load(memory_order_relaxed) == distance) {
								changed = true;
								break;
							}
							else
